...
namespace Pathfinding.RVO {
	...
	public class Simulator {
		...
		public void Update () {
			...
			if (Time.time - lastStep >= DesiredDeltaTime) {
				...
				if (Multithreading) {
					...
					// Find neighbours and calculate horizon avoidance
					for (int i = 0; i < workers.Length; i++) workers[i].Execute(0);
					for (int i = 0; i < workers.Length; i++) workers[i].WaitOne();

					for (int it = 0; it < 3; it++) {
						for (int i = 0; i < agents.Count; i++) {
							agents[i].CalculateHardCollisions();
						}
					}

					// Run second phase of the horizon avoidance
					for (int i = 0; i < workers.Length; i++) workers[i].Execute(4);
					for (int i = 0; i < workers.Length; i++) workers[i].WaitOne();

					// Calculate New Velocity
					for (int i = 0; i < workers.Length; i++) workers[i].Execute(3);
					...
					for (int i = 0; i < agents.Count; i++) {
						...
						agents[i].CalculateHorizonAvoidancePhase1(coroutineWorkerContext);
					}
					...
					for (int i = 0; i < agents.Count; i++) {
						agents[i].CalculateHorizonAvoidancePhase2(coroutineWorkerContext);
					}
		...
		internal class WorkerContext {
			...
			public float[] horizonAngleBuffer = new float[32];
			public int[] horizonEventBuffer = new int[32];
		}
					...
					try {
						...
						if (task == 0) {
							for (int i = start; i < end; i++) {
								agents[i].CalculateNeighbours();
								agents[i].CalculateHorizonAvoidancePhase1(context);
							}
						} else if (task == 1) {
							for (int i = start; i < end; i++) {
								agents[i].BufferSwitch();
							...
							for (int i = start; i < end; i++) {
								agents[i].CalculateHorizonAvoidancePhase2(context);
							}
						} else {
