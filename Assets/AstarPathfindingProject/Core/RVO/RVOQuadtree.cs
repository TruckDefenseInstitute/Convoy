...
namespace Pathfinding.RVO {
	...
	public class RVOQuadtree {
		...
		struct Node {
			...
			/// <summary>Maximum radius of all agents inside this node</summary>
			...
			public float CalculateMaxSpeed (Node[] nodes, int index) {
				if (child00 == index) {
					...
					for (var agent = linkedList; agent != null; agent = agent.next) {
						...
						maxRadius = System.Math.Max(maxRadius, agent.radius);
					}
				} else {
					...
					maxRadius = System.Math.Max(nodes[child00+0].maxRadius, nodes[child00+1].maxRadius);
					maxRadius = System.Math.Max(maxRadius, nodes[child00+2].maxRadius);
					maxRadius = System.Math.Max(maxRadius, nodes[child00+3].maxRadius);
				}
				return maxSpeed;
			}
		}
		...
		int GetNodeIndex () {
			...
			for (int i = 0; i < 4; i++) {
				nodes[filledNodes+i] = new Node();
				nodes[filledNodes+i].child00 = filledNodes+i;
			}
			filledNodes += 4;
		...
		internal void CalculateSpeeds () {
			nodes[0].CalculateMaxSpeed(nodes, 0);
		}

		public void FindAllTouchingCircle (Vector2 position, float radius, List<IAgent> resultBuffer) {
			FindAllTouchingCircleRec(0, bounds, position, radius, resultBuffer);
		}

		void FindAllTouchingCircleRec (int i, Rect r, Vector2 p, float radius, List<IAgent> buffer) {
			if (nodes[i].child00 == i) {
				// Leaf node
				for (Agent a = nodes[i].linkedList; a != null; a = a.next) {
					if ((a.Position - p).sqrMagnitude <= (radius + a.radius)*(radius + a.radius)) {
						buffer.Add(a);
					}
				}
			} else {
				var conservativeRadius = radius + nodes[i].maxRadius;
				// Not a leaf node
				Vector2 c = r.center;
				if (p.x-conservativeRadius < c.x) {
					if (p.y-conservativeRadius < c.y) {
						FindAllTouchingCircleRec(nodes[i].child00+0, Rect.MinMaxRect(r.xMin, r.yMin, c.x, c.y), p, radius, buffer);
					}
					if (p.y+conservativeRadius > c.y) {
						FindAllTouchingCircleRec(nodes[i].child00+1, Rect.MinMaxRect(r.xMin, c.y, c.x, r.yMax), p, radius, buffer);
					}
				}

				if (p.x+conservativeRadius > c.x) {
					if (p.y-conservativeRadius < c.y) {
						FindAllTouchingCircleRec(nodes[i].child00+2, Rect.MinMaxRect(c.x, r.yMin, r.xMax, c.y), p, radius, buffer);
					}
					if (p.y+conservativeRadius > c.y) {
						FindAllTouchingCircleRec(nodes[i].child00+3, Rect.MinMaxRect(c.x, c.y, r.xMax, r.yMax), p, radius, buffer);
					}
				}
			}
		}

		internal void Query (Vector2 p, float speed, float timeHorizon, float agentRadius, Agent agent) {
			new QuadtreeQuery {
